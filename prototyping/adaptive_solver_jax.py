...
def ode23(m, tol=0.0001, h_zee=0.0):
  ...
  avg_numerical_error = jax.numpy.sqrt(jax.numpy.sum(tau__n_p_1**2) / np.prod(tau__n_p_1.shape)) 
...
def ode45(m, tol=0.0001, h_zee=0.0):
  ...
  # Root Mean Squared Error 
  avg_numerical_error = jax.numpy.sqrt(jax.numpy.np.sum(tau__n_p_1**2) / np.prod(tau__n_p_1.shape)) 
...
def h_eff(m):
  # demag field
  m_pad[:n[0],:n[1],:n[2],:] = m
  f_m_pad = jax.numpy.fft.fftn(m_pad, axes = list(filter(lambda i: n[i] > 1, range(3))))
  ...
  h_demag = jax.numpy.fft.ifftn(f_h_demag_pad, axes = list(filter(lambda i: n[i] > 1, range(3))))[:n[0],:n[1],:n[2],:].real
...
f_n_demag = jax.numpy.fft.fftn(n_demag, axes = list(filter(lambda i: n[i] > 1, range(3))))
print("C")
...
alpha = 0.02
dt    = 2e-12
h_zee = jax.numpy.tile(np.array([-24.6e-3/mu0, +4.3e-3/mu0, 0.0]), np.prod(n)).reshape(m.shape)
...
with open(f'{output_filename}.dat', 'w') as f:
  ...
  while cum_time <= simulation_time:
    ...
    mx, my, mz = tuple(map(lambda i: jax.numpy.mean(m[:,:,:,i]), range(3)))
