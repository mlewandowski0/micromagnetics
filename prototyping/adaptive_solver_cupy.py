...
def dm_dt(m, h_zee = 0.):
    ...
    h = h_eff(m) + h_zee
    dmdt = - gamma/(1+alpha**2) * cp.cross(m, h) - alpha*gamma/(1+alpha**2) * cp.cross(m, cp.cross(m, h))
    return dmdt
...
def ode23(m, tol=0.0001, h_zee=0.0):
  ...
  avg_numerical_error = sqrt(cp.sum(tau__n_p_1**2) / np.prod(tau__n_p_1.shape)) 
...
def set_n_demag(c, permute, func):
  ...
  while not it.finished:
    value = 0.0
    for i in cp.rollaxis(cp.indices((2,)*6), 0, 7).reshape(64, 6):
...
def h_eff(m):
  # demag field
  m_pad[:n[0],:n[1],:n[2],:] = m
  f_m_pad = cp.fft.fftn(m_pad, axes = list(filter(lambda i: n[i] > 1, range(3))))
  f_h_demag_pad = cp.zeros(f_m_pad.shape, dtype=f_m_pad.dtype)
  ...
  h_demag = cp.fft.ifftn(f_h_demag_pad, axes = list(filter(lambda i: n[i] > 1, range(3))))[:n[0],:n[1],:n[2],:].real
  ...
  for i in range(6):
    ...
    v = cp.repeat(m, repeats, axis = i%3) / dx[i%3]**2
    h_ex += v
...
if calculate_demag_tensor:
  ...
  n_demag = cp.zeros([2*i-1 for i in n] + [6])
  for i, t in enumerate(((f,0,1,2),(g,0,1,2),(g,0,2,1),(f,1,2,0),(g,1,2,0),(f,2,0,1))):
    set_n_demag(i, t[1:], t[0])

  print(n_demag.shape)
  cp.save(demag_tensor_file, n_demag)
else:
  print(f"loading the demagnetization tensor from {demag_tensor_file}")
  n_demag = cp.load(demag_tensor_file)

print("B")
starting_time = time()
m_pad     = cp.zeros([2*i-1 for i in n] + [3])
f_n_demag = cp.fft.fftn(n_demag, axes = list(filter(lambda i: n[i] > 1, range(3))))
print("C")
...
alpha = 0.02
dt    = 2e-12
h_zee = cp.tile([-24.6e-3/mu0, +4.3e-3/mu0, 0.0], np.prod(n)).reshape(m.shape)
...
with open(f'data/{output_filename}.dat', 'w') as f:
  ...
  while cum_time <= simulation_time:
    ...
    mx, my, mz = tuple(map(lambda i: cp.mean(m[:,:,:,i]), range(3)))
